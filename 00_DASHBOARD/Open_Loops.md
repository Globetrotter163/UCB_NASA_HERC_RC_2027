...
FROM "02_Systems_Engineering"
WHERE type = "interface" AND status != "verified" AND status != "closed"
```
...
LIST
FROM "02_Systems_Engineering"
WHERE type = "decision" AND status != "rejected" AND (!evidence OR evidence = "")
```
...
WHERE type = "requirement" AND status != "retired" AND (!linked_tests OR length(linked_tests) = 0)
```
